...
void AP_Motors6DOF::setup_motors(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    case SUB_FRAME_CUSTOM: // custon frame added for our specific use case
        ...
        add_motor_raw_6dof(AP_MOTORS_MOT_3, -0.237, 0, -1.0f, 0, 0, -1.0f, 3);  // lateral front thruster (y)
        add_motor_raw_6dof(AP_MOTORS_MOT_4, -1.0f, -0.876, 0, 0.57, 0, 0, 4);   // updwards right thruster (z)
        add_motor_raw_6dof(AP_MOTORS_MOT_5, 0.994, -0.876, 0, 0.57, 0, 0, 5);   // updwards left thruster (z)
        add_motor_raw_6dof(AP_MOTORS_MOT_6, 0, 1.0f, 0, 1.0f, 0, 0, 6);         // updwards front thruster (z)
        break;
